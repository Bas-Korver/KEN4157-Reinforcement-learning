...
def make_env(env_id, idx, capture_video, run_name, gamma, gravity: float | None = None):
    def thunk():
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        print(f"-----------------------------\nIteration {iteration} of {args.num_iterations}\n-----------------------------")
    ...
    if args.save_model:
        ...
        for gravity in gravities:
            ...
            episodic_returns_list.append(np.concatenate(episodic_returns, axis=0))
        ...
        wandb.log({"eval/mean_episodic_return": wandb.plot.bar(table, "gravity", "mean_episodic_return", title="Mean Episodic Return")})

        data = [[x, y] for (x, y) in zip(gravity_keys, std_gravity_returns)]
        table = wandb.Table(data=data, columns=["gravity", "std_episodic_return"])
        wandb.log({"eval/std_episodic_return": wandb.plot.bar(table, "gravity", "std_episodic_return", title="Std Episodic Return")})

        data = [[x, y] for (x, y) in zip(gravity_keys, min_gravity_returns)]
        table = wandb.Table(data=data, columns=["gravity", "min_episodic_return"])
        wandb.log({"eval/min_episodic_return": wandb.plot.bar(table, "gravity", "min_episodic_return", title="Min Episodic Return")})

        data = [[x, y] for (x, y) in zip(gravity_keys, max_gravity_returns)]
        table = wandb.Table(data=data, columns=["gravity", "max_episodic_return"])
        wandb.log({"eval/max_episodic_return": wandb.plot.bar(table, "gravity", "max_episodic_return", title="Max Episodic Return")})
